...
export default function StoppageManagement({ stoppages, onAddStoppage, onDeleteStoppage, isAdmin }: StoppageManagementProps) {
  ...
  const [stoppageCode, setStoppageCode] = useState(STOPPAGES_LIST[0].code);
          ...
          >
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="p-6 grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6">
                ...
                {/* Legend Box from Physical Sheet */}
                <div className="md:col-span-4 bg-slate-50 border border-slate-200/80 rounded-xl p-4 mt-2 select-none">
                  <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-2 flex items-center space-x-1">
                    <Info className="w-3.5 h-3.5 text-slate-400" />
                    <span>Nº das paradas • Legenda Oficial Porto Brasil</span>
                  </h4>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-[11px] text-slate-600 font-medium">
                    {STOPPAGES_LIST.map(st => (
                      <div key={st.code} className="flex items-start space-x-1.5">
                        <span className="font-mono bg-slate-200/80 px-1.5 py-0.2 rounded text-slate-700 font-bold text-[10px] shrink-0">
                          {st.code}
                        </span>
