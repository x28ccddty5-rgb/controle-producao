...
const MANAGEMENT_LIST = ['Sara', 'Matheus', 'Jonas'];
...
export default function App() {
  ...
  // Application Authentication Sessions (2 roles: producao / supervisor)
  const [sessionUser, setSessionUser] = useState<'producao' | 'supervisor' | null>(() => {
    return (localStorage.getItem('porto_session_user') as 'producao' | 'supervisor') || null;
  });

  const [loginRole, setLoginRole] = useState<'producao' | 'supervisor'>('producao');
  const [loginPassword, setLoginPassword] = useState<string>('');
  const [loginError, setLoginError] = useState<string>('');

  const handleLoginSubmit = () => {
    const pwd = loginPassword.trim();
    if (loginRole === 'producao') {
      if (pwd === '1234') {
        setSessionUser('producao');
        localStorage.setItem('porto_session_user', 'producao');
        setActiveTab('ACTIVITIES');
        setLoginPassword('');
        setLoginError('');
      } else {
        setLoginError('Senha de Produção incorreta! Use "1234".');
      }
    } else {
      if (pwd === 'ADM2026') {
        setSessionUser('supervisor');
        localStorage.setItem('porto_session_user', 'supervisor');
        setActiveTab('DASHBOARD');
        setLoginPassword('');
        setLoginError('');
      } else {
        setLoginError('Senha de Supervisor incorreta! Use "ADM2026".');
      }
    }
  };

  const handleLogout = () => {
    if (confirm('Deseja realmente sair e desconectar deste terminal?')) {
      setSessionUser(null);
  ...
  const [activeTab, setActiveTab] = useState<string>(() => {
    ...
    return savedUser === 'supervisor' ? 'DASHBOARD' : 'ACTIVITIES';
  });
      ...
      <div className="min-h-screen bg-slate-950 flex items-center justify-center font-sans px-4 relative overflow-hidden" id="login-screen-wrapper">
        ...
        <div className="max-w-md w-full relative z-10">
          ...
          <div className="text-center mb-8">
            <div className="w-16 h-16 bg-blue-600 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-xl border border-blue-500/20 shadow-blue-500/10">
              <Factory className="w-8 h-8 text-white" />
            </div>
            <h1 className="text-2xl font-bold font-sans tracking-tight text-white uppercase">Porto Brasil</h1>
            <p className="text-slate-400 text-xs mt-1 font-mono uppercase tracking-widest">Painel de Movimentação & Paradas</p>
          </div>

          {/* Form Card */}
          <div className="bg-slate-900/90 border border-slate-800/80 rounded-2xl p-8 shadow-2xl backdrop-blur-md space-y-6">
            <div className="space-y-1 text-center">
              <h2 className="text-lg font-bold text-white">Identificação do Perfil</h2>
              <p className="text-xs text-slate-400">Escolha o nível de acesso para entrar no sistema</p>
            </div>

            {/* Error Message */}
            {loginError && (
              <div className="bg-rose-500/10 border border-rose-500/20 text-rose-300 text-xs p-3 rounded-xl font-medium text-center">
                ⚠️ {loginError}
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4 animate-fadeIn">
              <button
                type="button"
                onClick={() => {
                  setLoginRole('producao');
                  setLoginError('');
                }}
                className={`p-4 rounded-xl border text-center transition-all cursor-pointer flex flex-col items-center justify-center space-y-2 ${
                  loginRole === 'producao'
                    ? 'bg-blue-600/20 border-blue-500 text-white shadow-lg shadow-blue-500/5'
                    : 'bg-slate-805/40 border-slate-800 hover:border-slate-700 text-slate-400'
                }`}
              >
                <User className="w-5 h-5 shrink-0" />
                <div className="text-xs font-bold font-sans">Produção</div>
                <div className="text-[10px] text-slate-500 max-w-[120px] leading-tight shrink-0">Apontamento de lotes</div>
              </button>
              ...
              >
                ...
                <div className="text-xs font-bold font-sans">Supervisor</div>
                <div className="text-[10px] text-slate-500 max-w-[120px] leading-tight shrink-0">Dashboard Geral</div>
              </button>
            </div>

            {/* Password input */}
            <div className="space-y-2">
              <label className="block text-[10px] font-bold text-slate-300 uppercase tracking-widest">Senha de Segurança</label>
              <div className="relative">
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 hover:border-slate-700 rounded-xl px-4 py-3 text-white text-xs font-mono outline-hidden focus:border-blue-500 tracking-widest transition"
            ...
            <button
              ...
              className="w-full bg-blue-600 hover:bg-blue-500 text-white hover:shadow-lg hover:shadow-blue-500/10 font-bold py-3 px-4 rounded-xl text-xs transition duration-150 cursor-pointer text-center outline-hidden uppercase tracking-wider"
              id="btn-login-submit"
            >
              Confirmar Entrada
            </button>
          ...
          {/* Footer details */}
          <div className="text-center mt-6 text-slate-500 text-[10px] font-mono uppercase tracking-wider">
            PORTO-LOG v1.3.0 • Sistema Resiliente
          </div>
        </div>
      </div>
    );
            ...
            <div className="mt-8 p-4 bg-slate-800/80 border border-slate-700/80 rounded-xl space-y-4 shadow-inner">
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  className="w-full bg-slate-900 border border-slate-700 hover:border-slate-600 rounded-lg px-3 py-2 text-white text-xs outline-hidden focus:border-blue-500 cursor-pointer"
                >
                  {MANAGEMENT_LIST.map(m => (
